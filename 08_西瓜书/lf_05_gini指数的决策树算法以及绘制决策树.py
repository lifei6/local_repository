...
def isBlankA(A):  # 2.判断 A 是否是空，是空则返回true--属性是否都使用过
    for i in range(n):
        if A[i] > 0: return False
    return True


def isSameAinD(D, A):  # 3.判断在D中，是否所有的未使用过的样本属性均相同
    for i in range(n):
        if A[i] > 0:
...
def treeGenerate(D, A, title):
    ...
    # 3.是否所有属性全部使用过  或者  D中所有样本的未使用的属性均相同
...
if __name__ == '__main__':
    dataset = pd.read_excel('./data_sets/WaterMelon_2.0.xlsx')  # 读取数据
    # 一维数组存放特征名
    ...
    # 二维数组存放特征值
    dataset = dataset[:, :-1]
    m, n = np.shape(dataset)  # 得到数据集大小n包含了序号
    ...
    print('决策树ID',myDecisionTreeRoot.ID)
    # 绘制决策树
